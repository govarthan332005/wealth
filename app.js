...
(async function() {
  ...
  var _withdrawModalOpening = false; // race-condition guard
  var _configPollTimer = null;       // fallback polling for admin config
  var _plansPollTimer = null;        // fallback polling for admin plans
  ...
  function startPlatformConfigListener() {
    ...
    try {
      ...
      _platformConfigUnsub = onSnapshot(ref, function(snap) {
        ...
        // ═══ v17 FIX: refresh reward calendar so admin reward changes reflect ═══
        try { renderDailyCalendar(); } catch(e) {}
        try { loadDailyRewardStatus(); } catch(e) {}
      });
    } catch(err) {
    ...
    try {
      ...
      _plansConfigUnsub = onSnapshot(ref, function(snap) {
        ...
        if (data.plans && Array.isArray(data.plans) && data.plans.length > 0) {
          ...
          // ═══ v17 FIX: also refresh My Investments so plan name/return updates show ═══
  ...
  function _onUserSessionStart() {
    ...
    // ═══ FIX: Polling fallback for admin→user sync ═══
    // In local/demo mode onSnapshot only fires once, so poll every
    // 10s to detect admin changes to platform config + plans.
    startConfigPolling();
    startPlansPolling();
  }
  ...
  // ═══ v17 FIX: Polling fallback for admin config + plans ═══
  function startConfigPolling() {
    ...
    _configPollTimer = setInterval(function() {
      ...
      try {
        getDoc(doc(db, 'config', 'platform')).then(function(snap) {
          if (!snap || !snap.exists || !snap.exists()) return;
          var data = snap.data() || {};
          // detect change by JSON-comparing key fields
          var changed = false;
          var keys = ['minDeposit','minWithdraw','referralPercentage','withdrawFeePercent','baseReward','streakBonus','maxReward'];
          for (var i = 0; i < keys.length; i++) {
            if (Number(data[keys[i]]) !== Number(runtimePlatformConfig[keys[i]])) { changed = true; break; }
          }
          if (changed) {
            runtimePlatformConfig = Object.assign({}, DEFAULT_PLATFORM_CONFIG, data);
            try { applyRuntimeConfigToUI(); } catch(e) {}
            try { if (typeof updateWithdrawLivePreview === 'function') updateWithdrawLivePreview(); } catch(e) {}
            try { renderDailyCalendar(); } catch(e) {}
            try { loadDailyRewardStatus(); } catch(e) {}
            try { setCache('config', { platform: runtimePlatformConfig, upi: runtimeUpiConfig, deposit: runtimeDepositConfig, links: runtimeLinksConfig, withdraw: runtimeWithdrawConfig }); } catch(e) {}
          }
  ...
  function startPlansPolling() {
    stopPlansPolling();
    _plansPollTimer = setInterval(function() {
      if (!currentUser) return;
      if (document.hidden) return;
      try {
        getDoc(doc(db, 'config', 'plans')).then(function(snap) {
          if (!snap || !snap.exists || !snap.exists()) return;
          var data = snap.data() || {};
          if (!data.plans || !Array.isArray(data.plans) || !data.plans.length) return;
          // Detect change by stringifying
          var newStr = '';
          var oldStr = '';
          try { newStr = JSON.stringify(data.plans); } catch(e) {}
          try { oldStr = JSON.stringify(PLANS); } catch(e) {}
  ...
  function stopPlansPolling() {
    if (_plansPollTimer) { clearInterval(_plansPollTimer); _plansPollTimer = null; }
  }
  ...
  async function initUserDashboard() {
    // ═══ v17 FIX: Save original withdraw modal HTML AFTER DOM is fully ready ═══
    // Previously this could race — if the modal HTML wasn't fully parsed,
    // we'd save an empty string, causing the withdraw modal to glitch open.
    function _saveWithdrawModalSnapshot() {
      var wmBody = document.querySelector('#withdrawModal .modal-body');
      var wmFooter = document.querySelector('#withdrawModal .modal-footer');
      if (wmBody && wmBody.innerHTML && wmBody.innerHTML.length > 50 && !originalWithdrawModalBody) {
        originalWithdrawModalBody = wmBody.innerHTML;
      }
      if (wmFooter && wmFooter.innerHTML && wmFooter.innerHTML.length > 20 && !originalWithdrawModalFooter) {
        originalWithdrawModalFooter = wmFooter.innerHTML;
      }
    }
    _saveWithdrawModalSnapshot();
    // Re-attempt after a tick in case the DOM hadn't fully painted yet
    setTimeout(_saveWithdrawModalSnapshot, 100);
    setTimeout(_saveWithdrawModalSnapshot, 500);
  ...
  function renderPlans() {
    ...
    grid.innerHTML = PLANS.map(function(p, idx) {
      ...
      // v19: 16:9 aspect-ratio frame, image rendered with object-fit: contain
      // so the FULL image is always visible (no cropping). The frame itself
      // has a gradient background that acts as letterbox.
      var imageBlock;
      if (p.image) {
        imageBlock = '<div class="plan-media plan-media-16x9">' +
  ...
  function _renderInvestmentsList(invList, container) {
    ...
    container.innerHTML = invList.map(function(inv) {
      ...
      var totalReturn = dailyReturn * duration;
      var earnedSoFar = dailyReturn * daysCompleted;
      var daysLeft = Math.max(0, duration - daysCompleted);
      var statusLabel = inv.status === 'active' ? 'Active' : (inv.status === 'completed' ? 'Completed' : (inv.status || 'pending'));
      return '<div class="investment-card inv-card-premium">' +
        '<div class="inv-card-top">' +
          '<div class="inv-card-icon"><i class="' + (plan.icon || 'fas fa-chart-line') + '"></i></div>' +
          '<div class="inv-card-titles">' +
            '<div class="inv-card-name">' + (inv.planName || '—') + '</div>' +
            '<div class="inv-card-sub"><i class="fas fa-calendar-day"></i> ' + fmtDate(inv.startDate) + '</div>' +
          '</div>' +
          '<div class="inv-card-amount-wrap">' +
            '<div class="inv-card-amount">' + fmt(inv.amount) + '</div>' +
            '<span class="inv-card-status status-' + inv.status + '"><i class="fas fa-' + (inv.status === 'active' ? 'circle-play' : (inv.status === 'completed' ? 'circle-check' : 'clock')) + '"></i> ' + statusLabel + '</span>' +
          '</div>' +
        '</div>' +
        '<div class="inv-card-progress">' +
          '<div class="inv-card-progress-head">' +
            '<span><i class="fas fa-chart-line"></i> Progress</span>' +
            '<strong>' + progressPct + '%</strong>' +
          '</div>' +
          '<div class="inv-card-progress-track"><div class="inv-card-progress-fill" style="width:' + progressPct + '%"></div></div>' +
          '<div class="inv-card-progress-foot">' +
            '<span><i class="fas fa-check"></i> Day ' + daysCompleted + ' of ' + duration + '</span>' +
            '<span><i class="fas fa-hourglass-half"></i> ' + daysLeft + ' days left</span>' +
          '</div>' +
        '</div>' +
        '<div class="inv-card-stats">' +
          '<div class="inv-card-stat"><span><i class="fas fa-coins"></i> Daily</span><strong>₹' + dailyReturn + '</strong></div>' +
          '<div class="inv-card-stat"><span><i class="fas fa-sack-dollar"></i> Earned</span><strong>' + fmt(earnedSoFar).replace('.00','') + '</strong></div>' +
          '<div class="inv-card-stat"><span><i class="fas fa-bullseye"></i> Target</span><strong>' + fmt(totalReturn).replace('.00','') + '</strong></div>' +
        '</div>' +
      '</div>';
    }).join("");
  }
  ...
  function renderTransactions(transactions, containerId) {
    ...
    container.innerHTML = transactions.map(function(t) {
      var isCredit = t.type === "deposit" || t.type === "daily_return" || t.type === "daily_reward" || t.type === "referral_bonus" || t.type === "signup_bonus" || t.type === "admin_credit";
      ...
      else if (t.type === "daily_reward") { typeIcon = "daily_reward"; iconClass = "fa-gift"; label = t.description || "Daily Reward 🎁"; }
      else if (t.type === "referral_bonus") { typeIcon = "referral_bonus"; iconClass = "fa-user-group"; label = "Refer Bonus 🎁"; }
      else if (t.type === "signup_bonus") { typeIcon = "signup_bonus"; iconClass = "fa-star"; label = "Signup Bonus 🎉"; }
      else if (t.type === "admin_credit") { typeIcon = "admin_credit"; iconClass = "fa-circle-plus"; label = t.description || "Admin Credit"; }
      else if (t.type === "invest") { typeIcon = "invest"; iconClass = "fa-seedling"; label = "Investment" + (t.plan ? " — " + t.plan : ""); }
      else { typeIcon = "invest"; iconClass = "fa-seedling"; label = (t.description || "Transaction"); }
  ...
  window.openModal = function(id) {
    ...
    if (id === "withdrawModal") {
      // ═══ v17 FIX: Withdraw modal glitch — guard against re-entrancy + race ═══
      if (_withdrawModalOpening) return;
      _withdrawModalOpening = true;
      try {
        var wmBody = document.querySelector('#withdrawModal .modal-body');
        var wmFooter = document.querySelector('#withdrawModal .modal-footer');
        // Capture snapshot on first open (in case init didn't catch it)
        if (wmBody && wmBody.innerHTML && wmBody.innerHTML.length > 50 && !originalWithdrawModalBody && !wmBody.querySelector('.withdraw-disabled-banner')) {
          originalWithdrawModalBody = wmBody.innerHTML;
        }
        if (wmFooter && wmFooter.innerHTML && wmFooter.innerHTML.length > 20 && !originalWithdrawModalFooter && !wmFooter.querySelector('.btn-secondary.full')) {
          originalWithdrawModalFooter = wmFooter.innerHTML;
        }
        // RESTORE original HTML if the disabled banner is showing
        ...
        if (!runtimeWithdrawConfig.apiWithdrawEnabled) {
          ...
          // ═══ v17 FIX: ensure runtime config is loaded before showing preview ═══
          // Previously preview could show NaN if config hadn't loaded yet.
          var feeRate = Number(runtimePlatformConfig.withdrawFeePercent);
          if (isNaN(feeRate) || feeRate < 0) {
            runtimePlatformConfig.withdrawFeePercent = Number(DEFAULT_PLATFORM_CONFIG.withdrawFeePercent || 3);
          }
          ...
          // Trigger a fresh config fetch in background to ensure latest fee %
          try { loadRuntimeConfig(true).then(function() { try { updateWithdrawLivePreview(); } catch(e) {} }).catch(function(){}); } catch(e) {}
        }
      } catch(err) {
        console.warn('[withdraw modal open] glitch caught:', err);
      } finally {
        setTimeout(function() { _withdrawModalOpening = false; }, 200);
      }
    }
    if (id === "depositModal") {
      uploadedScreenshot = null;
